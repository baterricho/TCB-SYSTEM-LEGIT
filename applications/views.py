...
class IPApplicationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "admin":
            return IPApplication.objects.all()
        return IPApplication.objects.filter(applicant=user)
    ...
    def submit_application(self, request, pk=None):
        ...
        case = ApplicationSubmissionService.submit(application, request)
        return Response({"detail": "Application submitted.", "case_number": case.case_number, "case_id": case.id}, status=status.HTTP_201_CREATED)
